...
const key = '767b3f050b17f8c713e3835b18f57ff9';// '46799a1920f8b8914ad7d0a2db0096d1'; //
...
function getUrlBuildingV5(x, y, z, depth = 1) {
  ...
  for (let i = -depth; i <= depth; i += 1) {
    for (let j = -depth; j <= depth; j += 1) {
      ...
      res[[z, x + i, y + j].join('_')] = { z, x: x + i, y: y + j };
    }
  }
  res = _.values(res);
  const vs = _.values(o);
  const qstr = _.map(vs, v => v.join(',')).join(';');
  return { url: `http://vdata.amap.com/tiles?v=3&style=5&rd=1&t=${qstr}`, tiles: res };// &flds=building
}
...
function processTileV5(x, y, z, dx, dy) {
  const px = x * pSize;
  const py = y * pSize;
  x = px + dx;
  y = py + dy;
