...
module.exports = async (record, success, fail) => {
  const { json } = record;
  if (json.code === RETRY) {
    const page = parseInt(Math.random()*50);
    await record.urlModel.clean();
    await record.urlModel.upsert([{ url: `https://localbitcoins.com/buy-bitcoins-online/.json?page=${page}` }]);
    success();
  } else {
    ...
    if (json.data.ad_list.length > 0) {
      ...
      if (json.pagination.next) {
        ...
        await record.urlModel.clean();
        await record.urlModel.upsert([{ url: 'https://localbitcoins.com/buy-bitcoins-online/.json?page=1' }]);
      }
