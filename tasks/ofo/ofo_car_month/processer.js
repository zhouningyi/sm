...
const carUpsert = (datas) => {
  ...
  return `
  INSERT INTO public.ofo_car_months ( car_id, ordernum, lat, lng, center, "createdAt", "updatedAt" )
  VALUES ${values.join(',')}
  ON CONFLICT (car_id) DO UPDATE SET ordernum = EXCLUDED.ordernum,
  lat = EXCLUDED.lat, lng = EXCLUDED.lng, center = EXCLUDED.center,
  "createdAt" = public.ofo_car_months."createdAt", "updatedAt" = EXCLUDED."updatedAt";
  `;
}
...
const historyUpsert = (datas) => {
  ...
  return `
  INSERT INTO public.ofo_car_month_histories ( unique_id, car_id, time, lat, lng, center, "createdAt", "updatedAt" )
  VALUES ${values.join(',')}
  ON CONFLICT (unique_id) DO UPDATE SET unique_id = EXCLUDED.unique_id, time = EXCLUDED.time,
  lat = EXCLUDED.lat, lng = EXCLUDED.lng, center = EXCLUDED.center,
  "createdAt" = public.ofo_car_month_histories."createdAt", "updatedAt" = EXCLUDED."updatedAt";
  `;
}
...
module.exports = function (record, success, fail) {
  ...
  let users = [];
  let cars = [];
  console.log("cars", json.values.info.cars.length);
  ...
  promiseQuery.push(Models.ofo_car.sequelize.query(historyUpsert(items)));
  promiseQuery.push(Models.ofo_car.sequelize.query(userUpsert(users)));
