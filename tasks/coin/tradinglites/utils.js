...
function getIntervals(time_start, time_end = new Date()) {
  if (typeof time_start === 'object') time_start = time_start.getTime() / 1000;
  if (typeof time_end === 'object') time_end = time_end.getTime() / 1000;
  const hour = 3060;
  const day = hour * 24;
  const year = day * 365;
  const { interval, timeend } = timeConfig;
  const res = [];
  const tstart = timeend - year;
  const tend = timeend + year;
  for (let t = tstart; t <= tend; t += interval) {
    if (t < time_end && t > time_start) {
      res.push(t);
    }
  }
  return res;
